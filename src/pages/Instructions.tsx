...
const basicInstructions: InstructionItem[] = [
  {
    ...
    content: 'Нажмите кнопку «Новая запись» при появлении тревоги или панической атаки. Последовательно заполните 7 шагов самонаблюдения. Подробное описание переключает внимание от паники на осмысленное действие и помогает лучше понять триггеры. Будьте максимально детальны — чем подробнее запись, тем эффективнее работает техника.',
  },
  {
    id: 'edit',
    icon: 'edit',
...
const descriptionInstructions: InstructionItem[] = [
  ...
  },
  {
    ...
    title: '5. Мысли',
    content: 'Какие катастрофические мысли возникли? "У меня инфаркт", "я умру", "я схожу с ума", "я не справлюсь"? Чем подробнее вы опишете мысли, тем лучше - это переключает внимание и помогает потом оспорить иррациональные убеждения.',
  },
  {
    id: 'body',
    icon: 'favorite',
    ...
    content: 'Опишите максимально детально все телесные симптомы: сердцебиение, дыхание, головокружение, дрожь, потливость, онемение, тошнота. Где в теле? Насколько сильно? Подробное описание само по себе снижает интенсивность паники.',
  },
  {
    id: 'actions',
    icon: 'exercise',
    ...
    title: '7. Действия',
    content: 'Что вы делали, чтобы справиться с паникой? Дыхательные упражнения, техники заземления, обращение за поддержкой? Детальное описание действий помогает закрепить эффективные копинг-стратегии и переключает внимание от паники.',
  },
];
...
const aiInstructions: InstructionItem[] = [
  {
    ...
    content: 'Используйте функцию "Скопировать для AI", чтобы получить профессиональный анализ паттернов ваших панических атак. Вставьте скопированный текст в чат с ChatGPT или Claude. AI поможет выявить триггеры, катастрофические интерпретации и предложит техники когнитивной реструктуризации.',
  },
];
